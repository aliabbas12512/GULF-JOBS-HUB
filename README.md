...
| `NEXT_PUBLIC_SITE_URL` | The production URL (`https://www.sbtjobshub.online`), used for canonical URLs, sitemap, and structured data. |
...
6. Once live, add both `sbtjobshub.online` and `www.sbtjobshub.online` to the Vercel project under
   **Project Settings → Domains**, set `www.sbtjobshub.online` as the primary/production domain (so
   Vercel redirects the apex to it), and set `NEXT_PUBLIC_SITE_URL=https://www.sbtjobshub.online` to
   match.
